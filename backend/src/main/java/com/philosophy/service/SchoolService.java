...
public class SchoolService {
    ...
    public List<Content> getContentsBySchoolIdWithPriority(Long schoolId) {
        try {
            ...
            // 仅按内容所属流派筛选：只看内容本身挂载的流派（含子流派）。
            List<Content> allContents = contentRepository.findBySchoolIdsDirect(schoolIds);
            // 完全不依赖 user 字段，避免旧数据导致整页无内容。
            // 仅做基础排序：点赞数高的优先，其次按ID倒序。
            allContents.sort((c1, c2) -> {
                int byLike = Integer.compare(c2.getLikeCount(), c1.getLikeCount());
                if (byLike != 0) {
                    return byLike;
                }
                Long id1 = c1.getId() == null ? 0L : c1.getId();
                Long id2 = c2.getId() == null ? 0L : c2.getId();
    ...
    /**
     * 获取指定流派的内容（分页），包含该流派及其子流派下所有可见内容，并按优先级排序。
     * 优先级：管理员 > 版主 > 用户（用户按点赞数）。
     */
    @Transactional(readOnly = true)
    public Map<String, Object> getContentsBySchoolIdWithPriorityPaged(Long schoolId, int page, int size) {
        Map<String, Object> result = new HashMap<>();
        try {
            List<Content> allContents = getContentsBySchoolIdWithPriority(schoolId);
            if (allContents == null || allContents.isEmpty()) {
                result.put("contents", new ArrayList<>());
                result.put("hasMore", false);
                result.put("totalElements", 0L);
                result.put("totalPages", 0);
                result.put("currentPage", Math.max(0, page));
                return result;
            }
            ...
            List<Content> pageContents = new ArrayList<>(allContents.subList(from, to));

            result.put("contents", pageContents);
            ...
            logger.error("Error getting priority contents by school ID (paged): {} - {}", schoolId, e.getMessage(), e);
            result.put("contents", new ArrayList<>());
            result.put("hasMore", false);
            result.put("totalElements", 0L);
            return result;
